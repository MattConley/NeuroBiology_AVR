...
public static class WMG_Anim {
    ...
    /**/
    public static void animVec3(DOGetter<Vector3> getter, DOSetter<Vector3> setter, float duration, Vector3 animTo, Ease easeType = Ease.Linear)
    {
        Vector3[] tempAnim = { animTo };
        float[] tempDur = { duration };
        DOTween.ToArray(getter, setter, tempAnim, tempDur).SetEase(easeType).SetUpdate(false);
    }
